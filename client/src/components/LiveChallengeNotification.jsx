...
export default function LiveChallengeNotification({
  ...
  shortsOwned,      // { survivorName: count }
  onClose,
}) {
  ...
  for (const name of winners) {
    ...
    totalImpact -= (shortsOwned[name] || 0) * WIN_RATE;
  }
  for (const name of losers) {
    totalImpact -= (sharesOwned[name] || 0) * LOSS_RATE;
    totalImpact += (shortsOwned[name] || 0) * LOSS_RATE;
  }
  ...
  const hasPosition = winners.some(n => sharesOwned[n] || shortsOwned[n]) ||
                      losers.some(n => sharesOwned[n] || shortsOwned[n]);
